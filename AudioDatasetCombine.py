...
def map_labels(label, label_mapping):
    if label not in label_mapping:
        print(f"Label {label} not found in mapping. Available keys: {list(label_mapping.keys())}")
...
class AudioDatasetCombine:

    def __init__(self, csv_file, save_dir, sr=44100, augumentations=None,interpolate=False, interpolation=cv2.INTER_LINEAR,model_name=""):
    ...
    def create_dataset(self, file_paths, batch_size):
        ...
        return dataset
    def pad_feature(self,min_shape, feature):
    ...
    def _extract_feature(self, y, sr):
        melspectrogram = librosa.power_to_db(librosa.feature.melspectrogram(y=y, sr=sr, n_mels=64, n_fft=4096, hop_length=2048, fmin=300, fmax=15000))
        spectrogram = librosa.power_to_db(np.abs(librosa.stft(y, n_fft=2048, hop_length=1024)) ** 2)
        mfcc=librosa.feature.mfcc(y=y, sr=sr, n_mfcc=40)


        if self.interpolate:
            melspectrogram= self.interpolate_image(melspectrogram)
            spectrogram= self.interpolate_image(spectrogram)
            mfcc= self.interpolate_image(mfcc)
        mfcc = np.expand_dims(mfcc, axis=-1)
        spectrogram = np.expand_dims(spectrogram, axis=-1)
        melspectrogram = np.expand_dims(melspectrogram, axis=-1)
        combined_feature = np.concatenate([mfcc,
                                           spectrogram,
                                           melspectrogram], axis=-1)
